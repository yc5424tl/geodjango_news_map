...
RUN DATABASE_URL=$(heroku config:get DATABASE_URL -a geodjango-news-map) && export DATABASE_URL=${DATABASE_URL}

RUN CONN_STR_LIST=($(python parse_conn_str.py "${DATABASE_URL}" | tr -d '[],')) && export CONN_STR_LIST=${CONN_STR_LIST}

RUN export NEWS_MAP_DB_USER=${CONN_STR_LIST[0]} && \
export NEWS_MAP_DB_PW=${CONN_STR_LIST[1]} && \
export NEWS_MAP_DB_HOST=${CONN_STR_LIST[2]} && \
export NEWS_MAP_DB_PORT=${CONN_STR_LIST[3]} && \
export NEWS_MAP_DB_NAME=${CONN_STR_LIST[4]}
